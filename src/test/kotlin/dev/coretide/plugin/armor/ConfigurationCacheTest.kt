...
class ConfigurationCacheTest {
    ...
    @Test
    fun `enabled code stats reuses the configuration cache on a second run`(
        @TempDir dir: File,
    ) {
        // Every build checks whether the repository reports, through a value source the cache
        // re-evaluates rather than remembers; an unchanged answer must still reuse the entry.
        ArmorTestFixture.writeProject(dir, armorConfig = "    codeStats {\n        enabled = true\n    }")
        ArmorTestFixture.initGitRepository(dir)
        val environment = ArmorTestFixture.isolatedGitEnvironment

        val first = ArmorTestFixture.runWithEnvironment(dir, "help", "--configuration-cache", set = environment, unset = setOf("CI"))
        val second = ArmorTestFixture.runWithEnvironment(dir, "help", "--configuration-cache", set = environment, unset = setOf("CI"))

        assertContains(first.output, "code stats is enabled but this repository is not reporting")
        assertContains(second.output, "Configuration cache entry reused")
    }
